            ...
            <View style={styles.container}>
                <ScrollView>
                    ...
                    <Picker
                        ...
                        pickerTitle="请选择性别"
                    />
                    {/* <DatePicker labelStyle={styles.pickerStyle}
                                key="activityStartTimeEdit"
                                underlineColorAndroid='transparent'
                                placeholder={'请选择开始时间'}
                                value={activityInfo.activityStartTime ? activityInfo.activityStartTime : ''}
                                timeMode={'datetime'}
                                valueStyle={styles.pickerValueStyle}
                                labelText="开始时间"
                                startTime={new Date().format("yyyy-MM-dd hh:mm")}
                                lastTime={new Date(new Date().setFullYear(new Date().getFullYear() + 1)).format("yyyy-MM-dd hh:mm")}
                                period={60}
                                returnValueFormat="YYYY-MM-DD HH:mm"
                                onSelect={(date) => this._setValue('activityStartTime', date)}
                                editable={(this.state.type === 'add' || this.state.editable === true) ? true : false}
                            /> */}
                    <Region
                        labelText="地址"
                        property="addresss"
                    ...
                    />
                    <InputArea style={{ marginTop: 10, backgroundColor: '#fff' }}
